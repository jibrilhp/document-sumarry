...
class ChatBotRepository:
    ...
    def _get_routing_decision(self, text: str) -> str:
        ...
        if _needs_database_analysis(text):
    ...
    def create_chatbot(self, thread_id: str) -> CompiledGraph:
        ...
        # End paths
        chatbot.add_edges(
            start_key="simulate_sentiment", end_key="add_answer_to_conversation"
        )
        chatbot.add_edges(
            start_key="generate_chat_response", end_key="add_answer_to_conversation"
        )
        chatbot.add_edges(
            start_key="analyze_database", end_key="add_answer_to_conversation"
        )
    ...
    def __route_after_context(
        self, state: State
    ) -> Literal["simulate_sentiment", "generate_chat_response", "get_database_summary"]:
        question = state["conversation"][-1].content
        
        if _needs_database_analysis(question):
